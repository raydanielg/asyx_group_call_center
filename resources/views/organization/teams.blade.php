...
@section('title', 'Teams - AYS Call Center')
@section('page_title', 'Organization · Teams')
...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div>
        ...
        <p class="text-xs text-gray-500">Manage teams within departments</p>
    </div>
    <button onclick="openModal('modal-team')" class="px-3 py-2 text-xs font-medium bg-navy-600 text-white rounded-lg hover:bg-navy-700 transition-colors inline-flex items-center gap-1.5 self-start">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-t border-gray-100 hover:bg-gray-50/50 transition-colors" id="row-team-{{ $team->id }}">
                    <td class="px-5 py-3 font-medium text-gray-900">{{ $team->name }}</td>
                    <td class="px-5 py-3 text-gray-500">{{ $team->department?->name ?? 'N/A' }}</td>
                    <td class="px-5 py-3 text-gray-500">{{ $team->employees?->count() ?? 0 }}</td>
                    <td class="px-5 py-3">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium {{ $team->is_active ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-gray-50 text-gray-500 border border-gray-100' }}">{{ $team->is_active ? 'Active' : 'Inactive' }}</span>
                    </td>
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-end gap-1">
                            <button type="button" onclick="openEditTeam({{ $team->id }})" class="p-1.5 rounded-lg text-navy-600 hover:bg-navy-50 hover:text-navy-700 transition-colors" title="Edit">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
                            <form method="POST" action="{{ route('organization.teams.destroy', $team) }}" class="inline" data-ajax data-confirm="Delete this team?" data-row-id="row-team-{{ $team->id }}">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="5" class="px-5 py-10 text-center text-gray-400"><p class="text-sm">No teams yet</p></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div class="px-5 py-3 border-t border-gray-100">{{ $teams->links() }}</div>
</div>

<div id="modal-team" class="fixed inset-0 z-50 hidden">
    <div class="absolute inset-0 bg-black/40" onclick="closeModal('modal-team')"></div>
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-xl shadow-xl p-5">
        <h3 class="text-sm font-semibold text-gray-900 mb-4">Add Team</h3>
        <form method="POST" action="{{ route('organization.teams.store') }}" class="space-y-3" data-ajax data-close-modal="modal-team" data-reset-on-success="true">
            @csrf
            ...
            <div class="flex gap-2 pt-2">
                ...
                <button type="button" onclick="closeModal('modal-team')" class="px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
            </div>
        </form>
    </div>
</div>
...
{-- Edit Modal --}}
<div id="modal-team-edit" class="fixed inset-0 z-50 hidden">
    <div class="absolute inset-0 bg-gray-900/50 backdrop-blur-sm" onclick="closeModal('modal-team-edit')"></div>
    <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl mx-4">
        <div class="flex items-center gap-3 p-5 border-b border-gray-100">
            ...
            <h3 class="text-sm font-bold text-gray-900">Edit Team</h3>
            <button type="button" onclick="closeModal('modal-team-edit')" class="ml-auto p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        <form id="form-team-edit" method="POST" class="p-5 space-y-3" data-ajax data-close-modal="modal-team-edit">
            @csrf @method('PUT')
            ...
            <div class="flex gap-2 pt-2">
                ...
                <button type="button" onclick="closeModal('modal-team-edit')" class="px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">Cancel</button>
            </div>
        </form>
    </div>
</div>
...
(function() {
    ...
    window.openEditTeam = function(id) {
        ...
        if (!t) return;
        editForm.action = '{{ route("organization.teams.update", "__ID__") }}'.replace('__ID__', id);
        editForm.querySelector('[name="name"]').value = t.name || '';
        editForm.querySelector('[name="department_id"]').value = t.department_id || '';
