...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    ...
    <button onclick="document.getElementById('modal-dept').classList.remove('hidden')" class="px-3 py-2 text-xs font-medium bg-navy-600 text-white rounded-lg hover:bg-navy-700 transition-colors inline-flex items-center gap-1.5 self-start">
            ...
            <tbody>
                ...
                <tr class="border-t border-gray-100 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-5 py-3 text-right">
                        <form method="POST" action="{{ route('organization.departments.destroy', $dept) }}" class="inline" onsubmit="return confirm('Delete this department?')">
                            @csrf @method('DELETE')
...
<div id="modal-dept" class="fixed inset-0 z-50 hidden">
    <div class="absolute inset-0 bg-black/40" onclick="document.getElementById('modal-dept').classList.add('hidden')"></div>
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-xl shadow-xl p-5">
        ...
        <form method="POST" action="{{ route('organization.departments.store') }}" class="space-y-3">
            ...
            <div class="flex gap-2 pt-2">
                ...
                <button type="button" onclick="document.getElementById('modal-dept').classList.add('hidden')" class="px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
            </div>
        </form>
    </div>
</div>
