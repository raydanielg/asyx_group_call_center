    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr id="emp-row-{{ $emp->id }}" class="border-t border-gray-100 hover:bg-gray-50/50 transition-colors">
                    <td class="px-5 py-3">
                    ...
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-end gap-1">
                            <a href="{{ route('employees.show', $emp) }}" title="View Profile" class="p-1.5 rounded-lg text-navy-600 hover:bg-navy-50 hover:text-navy-700 transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                            <a href="{{ route('employees.edit', $emp) }}" title="Edit Employee" class="p-1.5 rounded-lg text-gray-600 hover:bg-gray-100 hover:text-gray-800 transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            @if(!in_array($emp->employment_status, ['terminated','resigned']))
                            <button type="button" title="Delete Employee" data-delete-url="{{ route('employees.destroy', $emp) }}" data-confirm="Delete {{ $emp->first_name }} {{ $emp->last_name }}?" data-confirm-text="Yes, delete!" data-row-id="emp-row-{{ $emp->id }}" class="p-1.5 rounded-lg text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors">
